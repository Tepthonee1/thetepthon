...
LOGS = logging.getLogger("اعداد سيـفت")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@FFP8F"))
        await sbb_b(JoinChannelRequest("@SS22TT"))
        await sbb_b(JoinChannelRequest("@Safet_Help"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/5a5e0405ab4cc48e8da63.jpg",
                caption="**تم تشغيل سورس سيـفت بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/Safet_Help"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "⪼ هذه هي مجموعه الحفظ الخاصه بك لا تحذفها ابدا @FFP8F 𓆰."
        photobt = await sbb_b.upload_file(file="razan/pic/tepthon.jpeg")
        _, groupid = await create_supergroup(
            "كروب بوت سيـفت", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        descript = "❃ لا تحذف او تغادر المجموعه وظيفتها حفظ رسائل التي تأتي على الخاص @SS22TT"
        photobt = await sbb_b.upload_file(file="razan/pic/tepthon.jpeg")
        _, groupid = await create_supergroup(
